...
const Statistics: React.FC<StatisticsProps> = ({ onSectionChange }) => {
  ...
  const stats = [
    { value: '3000+', label: 'Community Members', color: 'bg-blue-300', borderColor: 'border-blue-500' },
    { value: '25+', label: 'Upcoming Projects', color: 'bg-pink-300', borderColor: 'border-pink-500' },
    { value: '20+', label: 'Events Hosted', color: 'bg-purple-300', borderColor: 'border-purple-500' },
  ];
        ...
        <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-10">
          {stats.map((stat, i) => (
            ...
            >
              ...
              <div className={`${stat.color} ${stat.borderColor} border-2 mt-4 text-black text-base md:text-lg font-medium tracking-wide text-center lowercase rounded-full px-2 py-0.5 inline-block`}>
                {stat.label}
              </div>
            </div>
          ))}
